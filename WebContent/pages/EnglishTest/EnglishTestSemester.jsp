...
<html>
    <head>
		...
		<title>英语考试管理考期管理</title>
    </head>
    <body>
		<fieldset class="layui-elem-field layui-field-title site-title">
			<legend>
				<a name="color-design">英语考试管理考期管理</a>
			</legend>
		</fieldset>
		...
		<fieldset class="layui-elem-field">
			<legend>英语考试管理考期管理</legend>
			<form class="layui-form" action="">
			  <div class="layui-form-item" style="width: 300px;">
			    <label class="layui-form-label">学期:</label>
			    <div class="layui-input-block">
			      <input type="text" name="name" required  lay-verify="required" placeholder="输入学期" autocomplete="off" class="layui-input">
			    </div>
			  </div>
	...
	<script>
		...
		layui.use(['table','form'], function() {
			...
			var tableIns = table.render({
				...
				method: 'post',
				url: '/EnglishTestController/findSemester',
				width: 1000
					//,height: 274
					,
				cols: [
...
//						},
					...
					table.on('tool(findZhenShuManage)', function(obj) {
						...
						if(obj.event === 'del') {
							layer.confirm('真的删除行么', function(index) {
								...
								$.ajax({
									type: "post",
									url: "/EnglishTestController/updateSemesterStatus",
									data: {
										id: data.id
									},
									async: true,
							...
							//
								var updateFileFolderindex = layer.open({
									...
									yes: function() {
										...
										$.ajax({
											type: "post",
											url: "/EnglishTestController/updateSemesterName",
											data: {
												id: data.id,
												name:fileRemarks
											},
		  ...
		  form.on('submit(formDemo)', function(data){
		    //layer.msg(JSON.stringify(data.field));
								$.ajax({
									type: "post",
									url: "/EnglishTestController/insertSemester",
									data: {
