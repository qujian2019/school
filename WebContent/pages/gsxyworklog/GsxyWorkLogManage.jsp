...
<html>
    <head>
			<title>湖南工商职业学院员工工作日志列表管理</title>
    ...
    <body>
		...
		<fieldset class="layui-elem-field layui-field-title site-title">
			<legend>
				<a name="color-design">湖南工商职业学院员工工作日志列表管理</a>
			</legend>
		</fieldset>
		<blockquote class="layui-elem-quote">
			<a href="javascript:;" class="layui-btn" id="GsxyWorkLogA">
				<i class="layui-icon">&#xe612;</i> 人员工作总结
			</a>
		...
		<fieldset class="layui-elem-field">
			...
			<div class="demoTable">
				<form class="layui-form" action="" style="width: 300px;">
				  ...
				  <div class="layui-form-item">
				    <label class="layui-form-label">日志种类:</label>
				    <div class="layui-input-block">
				      <select name="workLogStatus" id="workLogStatus" lay-verify="" lay-filter="workLogStatus">
				        <option value="1">员工日志</option>
				        <option value="2">总结日志</option>
				      </select>
				    </div>
				  </div>
			  </form>
				...
				<div class="layui-inline">
					<input class="layui-input" name="workName" id="workName" />
				</div>
				查询数据的时间范围:
				开始时间-
				<div class="layui-inline">
				...
				月周数:
				<div class="layui-inline">
					<input class="layui-input" name="weeks" id="weeks" />
				</div>
			...
			<script type="text/html" id="StatusTpl">
				{{# if(d.workLogStatus == 1){ }}
				<span style="color: #C12E2A;">员工日志</span> 
				{{# }else if(d.workLogStatus == 2){ }}
				<span style="color: #376B29;">总结日志</span> 
				{{# } }}
			</script>
			<!--	描述：控制按钮 -->
...
//});
		...
		layui.use(['table','form'], function() {
				...
				,
					...
					table.on('tool(tableId)', function(obj) {
						...
						if(obj.event === 'del') {
								...
								$.ajax({
									...
									success: function(data) {
										if(data > 0) {
											...
											tableIns.reload({
												where: {
													workLogStatus:$("#workLogStatus").val()
												}
											});
							...
							window.location.href = "GsxyWorkLogSummary.jsp?workLogId=" + data.workLogId;
			...
			form.on('select(workLogStatus)', function(data){
			  //console.log(data.value); //得到被选中的值 
			  if(data.value == '1'){
			  	$("#GsxyWorkLogA").show();
							tableIns.reload({
			...
			//人员工作总结
			$("#GsxyWorkLogA").on("click", function() {
				var checkStatus = table.checkStatus('tableId'),
					data = checkStatus.data;
				...
				if(temp == '' || temp == null || temp == undefined){
					...
					location.href = "GsxyWorkLogSummary.jsp?temp="+temp;
				}
