...
import com.hngsxy.web.bean.user.UserInfo;
...
public class UserController {
	...
	public String findUserOne(User user, HttpSession session, HttpServletRequest req,
	...
	public @ResponseBody UserInfo findUserInfo(User user) {
		User temp = null;
		UserInfo userInof = new UserInfo();
		try {
			temp = userBizImpl.findUserById(user);
			userInof.setIdCard(temp.getIdCard());
			userInof.setUserAge(temp.getUserAge());
			userInof.setUserCode(temp.getUserCode());
			userInof.setUserGender(temp.getUserGender());
			userInof.setUserName(temp.getUserName());
			userInof.setUserPassword(temp.getUserPassword());
			userInof.setUserPhone(temp.getUserPhone());
		} catch (Exception e) {
